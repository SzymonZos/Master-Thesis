#ifndef JPEG2000_LUTS_HPP
#define JPEG2000_LUTS_HPP
...
#define INSTANTIATE_LUT(lut) \
    constexpr inline auto lut##_f = detail::reverse_lut(detail::lut<float>); \
    constexpr inline auto lut##_d = detail::reverse_lut(detail::lut<double>)
...
template<typename T>
constexpr inline auto lut_db1 = std::to_array<T>(
    {7.071067811865475244008443621048490392848359376884740365883398e-01,
     7.071067811865475244008443621048490392848359376884740365883398e-01});

template<typename T>
constexpr inline auto lut_db2 = std::to_array<T>(
...
template<typename T>
constexpr inline auto lut_db3 = std::to_array<T>(
